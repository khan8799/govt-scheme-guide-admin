      ...
      try {
        ...
        const [statesRes, categoriesRes] = await Promise.all([
          fetch(`${API_BASE_URL}/user/getAllStates`, { 
            ...
            cache: 'no-store' // Add cache control to prevent 304 issues
          }),
  ...
  const loadSchemes = useCallback(async (page: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      const res = await fetch(url, { 
        headers: authHeaders2 as HeadersInit,
        cache: 'no-store' // Add cache control to prevent 304 issues
      });
      if (!res.ok) {
        if (res.status === 404) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try { const errorData = await res.json(); message = errorData.message || message; } catch {}
              ...
              <div className="p-6 text-sm text-gray-600">
                {selectedState || selectedCategory ? (
                  <div className="text-center">
                    ...
                    <button onClick={() => { resetFilters(); loadSchemes(1, false); }} className="mt-2 px-4 py-2 bg-brand-500 text-white rounded-md hover:bg-brand-600">
            ...
            {filteredSchemes.length > 0 && (
              <div className="p-4 border-t border-gray-200">
                <div className="flex items-center justify-between">
                  ...
                  {totalPages > 1 && (
                    <div className="flex gap-2">
                      <button onClick={() => loadSchemes(currentPage - 1, false)} disabled={currentPage <= 1} className="px-3 py-1 text-sm bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed">
                        Previous
                      </button>
                      <span className="px-3 py-1 text-sm text-gray-600">Page {currentPage} of {totalPages}</span>
                      <button onClick={() => loadSchemes(currentPage + 1, false)} disabled={!hasMore} className="px-3 py-1 text-sm bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed">
                        Next
                      </button>
                    </div>
                  )}
                ...
                <div className="mt-3 text-center space-y-2">
                  {hasMore && (
                    <button onClick={() => loadSchemes(currentPage + 1, true)} disabled={loading} className="px-4 py-2 bg-brand-500 text-white rounded-md hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="mt-6 pt-6 border-t border-gray-200 text-right">
                <button onClick={() => setSelectedScheme(null)} className="px-4 py-2 bg-brand-500 text-white rounded-md hover:bg-brand-600">Close</button>
              </div>
            </div>
          </div>
        </div>
